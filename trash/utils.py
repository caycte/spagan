...
def load_data_orggcn(dataset):
    ...
    test_idx_reorder = parse_index_file(f"data/{dataset}/ind.{dataset}.test.index")
    test_idx_range = np.sort(test_idx_reorder)
    
    if dataset == 'citeseer':
        # Fix {dataset} dataset (there are some isolated nodes in the graph)
    ...
    G = nx.DiGraph(graph)
    #inf= pickle.load(open('adj_{dataset}.pkl', 'rb'))
   
    print('astar length')
    for node_i,path in tqdm(nx.shortest_path_length(G)):
        for node_j, path_len in path.items():                
            adj_delta[node_i][node_j] = path_len
    update_or_create_pickle("data/adsf/","dijkstra_{dataset}.pkl",adj_delta)
    # a = open(f"data/adsf/dijkstra_{dataset}.pkl", 'wb')
    # pickle.dump(adj_delta, a)
   #######


    fw = open(f'data/adsf/ri_index_c_0.5_{dataset}_highorder_1_x_abs.pkl', 'rb')
    ri_index = pickle.load(fw)
    fw.close()

    fw = open(f'data/adsf/ri_all_c_0.5_{dataset}_highorder_1_x_abs.pkl', 'rb')
...
def load_pathm(dataset='cora', matpath=''):
...
def structural_interaction(ri_index, ri_all, g):
    """structural interaction between the structural fingerprints for {dataset}"""
